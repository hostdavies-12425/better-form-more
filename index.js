...
module.exports = class RoleColorEverywhere extends Plugin {
	startPlugin() {
		for (const category in replacements) {
			powercord.api.commands.registerCommand({
				...
				executor: (args) => ({
					send: true,
					result: this.doReplacement(args.join(' '), category),
				}),
			});
		}
	}
	...
	makeBinary(text) {
		text = text.split('');
		return text.map((char) => char.charCodeAt(0).toString(2)).join(' ');
	}
	...
	doReplacement(text, category) {
		if (category === 'varied') {
			return this.makeVaried(text);
		}
		if (category === 'binary') return this.makeBinary(text);
		if (category === 'base64') return this.makeBase64(text);
